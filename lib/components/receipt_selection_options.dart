...
class ReceiptSelectionOptions extends ConsumerWidget {
  ...
  const ReceiptSelectionOptions({required this.currDocType, super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final receipts = ref.watch(receiptsProvider(currDocType));
    ...
    return Row(
      children: [
        ActionChip(
          ...
          label: selectedReceipts.length == receipts.requireValue.length
          ...
          onPressed: () {
            if (selectedReceipts.length == receipts.requireValue.length) {
              selectedReceiptsMethods.clearSelectedReceipts();
            } else {
              selectedReceiptsMethods.selectAllReceipts(currDocType);
            }
          },
        ),
        ActionChip(
